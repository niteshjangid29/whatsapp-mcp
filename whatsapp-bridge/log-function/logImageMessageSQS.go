...
func LogImageMessageSQS(senderPhone string, text string, recipientPhone string, filePath string, messageTime time.Time, adminPhone string, msgId string, parMsgId string) error {
	...
	// Fetch the image from S3 with a timeout
	...
	if err != nil {
		log.Printf("❌ Error sending image log for message %s: %v", msgId, err)
		return fmt.Errorf("error sending log: %w", err)
	}
	defer apiResp.Body.Close()

	if apiResp.StatusCode != http.StatusOK {
		log.Printf("❌ Error response from log API for image message %s: %s", msgId, apiResp.Status)
		return fmt.Errorf("error response from log API: %s", apiResp.Status)
	}

	log.Printf("✅ Successfully logged image message %s", msgId)
	return nil
}
